...
void dividing_circle(double epsilon, Point center, double delta){
    ...
    if(firsttt==1){
        ...
        for(int i=0;i<circle_points.size();i++){
            myfile <<circle_points[i].first<<","<<circle_points[i].second<<"\n";
        }
...
vector<pll> removing_point(vector<pll> points,pll on_perimeter,pdd center){
    int answer;
    vector<pll> new_points;
    for(int i=0;i<int(points.size());i++){
...
void practical_way(vector<pll> points,int dep){
    ...
    vector<pll> new_points;//removing_point(points,selected_points[dep-1],circle_points[dep]);///That point which locates in a circle is removed
    int answer;
    for(int i=0;i<int(points.size());i++){
        ...
        if(answer==1||answer==2){
            ...
            new_points.clear();
            new_points=removing_point(points,points[i],circle_points[dep]);///That point which locates in a circle is removed
...
int main()
{
    ...
    int MaxX=10;
    int MaxY=10;
    int Max_of_member_color=5;
    int My_color_size=8;
    int max_member_subset=8;
    ...
    for(int i=0;i<key_size;i++){
        ...
        if(firsttt==0){
            cout<<firsttt<<endl;
            cout<<delta_dist_points.size()<<endl;
            for(int o=0;o<delta_dist_points.size();o++){
                myfile <<delta_dist_points[o].first<<","<<delta_dist_points[o].second<<"\n";
            }
        ...
        for(int k=0;k<int(delta_dist_points.size());k++){
            ...
            new_points.clear();
            new_points=removing_point(delta_dist_points,delta_dist_points[k],circle_points[0]);
